...
#     PROMPT_BOTH = PromptTemplate.from_template("""
# You are a helpful assistant combining BOTH video transcript and KB (PDF/text) information.
# User question: {question}
# Short-term memory (recent conversation):
# {memory_context}
# Long-term memory:
...
# {kb_context}
# Instructions:
# - Merge both sources into a single, **long, detailed, and fluent** answer.
# - Treat the video as the main explanation style; use KB to add structure, examples, or depth.
# - Avoid redundancy; if both mention the same concept, express it once naturally.
# - Do NOT say "according to the video" or "the PDF states".
# - If one source lacks info, rely on the other.
# - If both lack info, respond EXACTLY: "The answer is not present in the documents."
# """)
    ...
    # if video_context and not kb_context:
    #     print(" Video-only context detected")
    #     input_text = PROMPT_VIDEO_ONLY.format(
    #         question=query,
    #         memory_context=memory_context,
    #         long_term=video_mem,
    #         video_context=video_context
    #     )
    #     raw = llm.invoke(input_text)
    #     from_video = getattr(raw, "content", str(raw)).strip()
    #     from_kb = from_video  # identical for video-only
    
    # #  KB-only sources
    # elif kb_context and not video_context:
    #     print("KB-only context detected")
    #     input_text = PROMPT_KB_ONLY.format(
    #         question=query,
    #         memory_context=memory_context,
    #         long_term=kb_mem,
    #         kb_context=kb_context
    #     )
    #     raw = llm.invoke(input_text)
    #     from_kb = getattr(raw, "content", str(raw)).strip()
    #     from_video = "The answer is not present in the documents."

    # # 🟢 Both video + KB available
    # elif video_context and kb_context:
    #     print("🟢 Both video + KB context detected")
    #     input_text = PROMPT_BOTH.format(
    #         question=query,
    #         memory_context=memory_context,
    #         long_term=f"{video_mem}\n{kb_mem}",
    #         video_context=video_context,
    #         kb_context=kb_context
    #     )
    #     raw = llm.invoke(input_text)
    ...
    #     # Generate strong video-style base
    #     input_video = PROMPT_VIDEO_ONLY.format(
    #         question=query,
    #         memory_context=memory_context,
    #         long_term=video_mem,
    #         video_context=video_context
    #     )
    #     raw_video = llm.invoke(input_video)
    #     from_video = getattr(raw_video, "content", str(raw_video)).strip()
       
        # If KB answer is similar or shorter, unify them
    #     try:
    #         sim = SequenceMatcher(None, from_video.lower(), from_kb.lower()).ratio()
    #     except Exception:
    #         sim = 0.0
    #     if len(from_kb) < 250 or sim > 0.8:
    #         from_kb = from_video
...
# def answer_from_docs(query: str, docs: List[Document], llm, source_type: str) -> Dict[str, str]:
#     """
#     Generates long, structured answers using video and KB context.
#     - If the video context has relevant info, both video and KB answers are identical.
#     - If KB adds new, non-video info, KB expands slightly.
#     - If no relevant info, returns "The answer is not present in the documents."
#     """
...
#     if video_QA_index:
#         try:
#             vdocs = video_QA_index.similarity_search(query, k=1)
#             if vdocs:
#                 video_mem = vdocs[0].page_content
#         except Exception:
#             pass
#     if kb_QA_index:
#         try:
#             kdocs = kb_QA_index.similarity_search(query, k=1)
#             if kdocs:
#                 kb_mem = kdocs[0].page_content
#         except Exception:
#             pass
...
#     # --- Prompts (final tutorial-style ones) ---
#     PROMPT_VIDEO_ONLY = PromptTemplate.from_template("""
# You are a helpful and knowledgeable assistant answering questions based ONLY on the provided video transcript.
# User question: {question}
# Short-term memory (recent conversation):
...
# {video_context}
# Instructions:
# - Provide a **detailed, comprehensive, and well-structured** answer that explains the concept clearly.
# - Use **educational, tutorial-style language** — include examples, definitions, and relevant context from the video.
# - Preserve the **original phrasing and explanations** from the video when possible.
# - If the answer is not present in the video, respond EXACTLY: "The answer is not present in the documents."
# - NEVER mention PDFs, documents, or other external sources.
# - Be natural and fluent — don’t sound robotic or repetitive.
# """)
...
# Short-term memory (recent conversation):
# {memory_context}
# Long-term memory (previously answered Q&A from knowledge base):
# {long_term}
# Knowledge Base context (your ONLY source of truth):
# {kb_context}
# Instructions:
# - Provide a **comprehensive, factual, and clearly written** answer using information only from the KB.
# - Include examples, categories, or applications if they appear in the context.
# - DO NOT mention the video or any external knowledge.
# - If the KB does not contain the answer, reply EXACTLY: "The answer is not present in the documents."
# - Be fluent, complete, and informative — your tone should match an expert explaining to a learner.
# """)

#     PROMPT_BOTH = PromptTemplate.from_template("""
# You are a helpful and precise assistant answering questions using BOTH the video transcript and the knowledge base (PDF/text documents).

# User question: {question}
# Short-term memory (recent conversation):
# {memory_context}
# Long-term memory (previously answered Q&A):
# {long_term}
# Video context:
# {video_context}
# Knowledge Base context:
...
# Instructions:
# - Combine BOTH sources into a **single, detailed, and natural explanation**.
# - Treat the **video** as your primary guide for tone and phrasing, and the **KB** as a factual enhancement (for examples, structure, or definitions).
# - Merge overlapping details smoothly — do not repeat identical points.
# - If both contexts explain the same idea, summarize it clearly once using the most natural phrasing.
# - Include helpful elaborations (like examples, definitions, or use cases) from either source.
# - DO NOT mention “the video” or “the document.”
# - If only one source has relevant info, use that fully.
# - If neither contains the answer, respond EXACTLY: "The answer is not present in the documents."
# - Write in a natural, explanatory, and complete way — like an instructor giving a short tutorial.
# """)

#     # --- Generate answers ---
#     from_video, from_kb = "The answer is not present in the documents.", "The answer is not present in the documents."

#     # 1️⃣ VIDEO ONLY
#     if video_context and not kb_context:
#         print("for video only ")
#         input_video = PROMPT_VIDEO_ONLY.format(
#             question=query,
#             memory_context=memory_context,
#             long_term=video_mem,
...
#         raw_video = llm.invoke(input_video)
#         from_video = getattr(raw_video, "content", str(raw_video)).strip()
#         from_kb = from_video  # force identical if only video info

#     # 2️⃣ KB ONLY
#     elif kb_context and not video_context:
#         print("from pdf only ")
#         input_kb = PROMPT_KB_ONLY.format(
#             question=query,
#             memory_context=memory_context,
#             long_term=kb_mem,
#             kb_context=kb_context
#         )
#         raw_kb = llm.invoke(input_kb)
#         from_kb = getattr(raw_kb, "content", str(raw_kb)).strip()
#         from_video = "The answer is not present in the documents."
...
#             kb_context=kb_context
#         )
#         raw_kb = llm.invoke(input_both)
#         from_kb = getattr(raw_kb, "content", str(raw_kb)).strip()

#         # If KB answer is short or similar, force same as video
#         try:
#             sim = SequenceMatcher(None, from_video.lower(), from_kb.lower()).ratio()
#         except Exception:
#             sim = 0.0

#         if len(from_kb) < 250 or sim > 0.80:
#             from_kb = from_video

#     # 4️⃣ No context at all
#     else:
#         from_video = "The answer is not present in the documents."
#         from_kb = "The answer is not present in the documents."
...
# # =========================
# def get_kb_answer(query: str, combined_index_tuple: Tuple[Any, Dict[str, Any]], llm, embeddings,
#                   top_k=12, rerank_k=4, neighbor_window=2) -> Dict[str, Any]:
#     global kb_QA_index
#     out = {"answer": "The answer is not present in the documents.", "sources": []}
...
#         print("*********not in comnine index ")
#         return out

#     docs = combined_index.similarity_search(query, k=min(top_k, 30))
#     if not docs:
#         return out
...
#     # --- Identify if this KB is video-derived ---
#     has_video_source = any(d.metadata.get("source_type") == "video" for d in expanded)
#     has_kb_source = any(d.metadata.get("source_type") != "video" for d in expanded)

#     # Step 5: generate unified answer
#     # ans_dict = answer_from_docs(query, expanded, llm, source_type="knowledge_base")
#     # ans = ans_dict["kb"]
#     # print("kb",ans)

# # --- Step 5: Determine source dominance ---
#     has_video_source = any(d.metadata.get("source_type") == "video" for d in expanded)
#     has_kb_source = any(d.metadata.get("source_type") != "video" for d in expanded)

#     # --- If it's video-derived KB content, use only video docs ---
#     if has_video_source and not has_kb_source:
#         print("⚠️ KB is video-derived → using video-only docs for answer")
#         video_only_docs = [d for d in expanded if d.metadata.get("source_type") == "video"]
#         ans_dict = answer_from_docs(query, video_only_docs, llm, source_type="video")
#         ans = ans_dict["video"]
#     else:
#         print("🧩 True KB or mixed content detected → using normal both-context logic")
#         ans_dict = answer_from_docs(query, expanded, llm, source_type="knowledge_base")
#         ans = ans_dict["kb"]
        


#     # Step 6: Enforce video parity
#     if has_video_source and not has_kb_source:
#         # KB built from video → use video answer directly
#         ans = ans_dict["video"]

#     if ans == "The answer is not present in the documents.":
#         return out

#     # Step 7: save into KB QA memory
#     add_to_QA_index(query, ans, "knowledge_base", embeddings)
...
#                   top_k=12, rerank_k=4, neighbor_window=2) -> Dict[str, Any]:
#     print("inside get_kb_answer (PDF index, same as video logic)")
#     global kb_QA_index
#     out = {"answer": "The answer is not present in the pdfs."}

#     # Step 1: check long-term QA memory
#     if kb_QA_index:
#         print("kb qa index is present")
#         qa_docs = kb_QA_index.similarity_search(query, k=1)
#         if qa_docs and qa_docs[0].page_content:
#             print("short teram memory found ")
#             out["answer"] = qa_docs[0].page_content
#             out["sources"] = ["KB_QA_Memory"]

#             return out

#     # Step 2: retrieve from PDF vector index
#     pdf_index, doc_store = pdf_index_tuple if isinstance(pdf_index_tuple, tuple) else (pdf_index_tuple, {})
#     if not pdf_index:
#         print("NO PDF index***********************************")
#         return out

#     docs = pdf_index.similarity_search(query, k=min(top_k, 20))
#     if not docs:
#         print("No docs found *****************************")
#         return out

#     # Step 3: rerank results (identical to video method)
#     try:
#         if use_crossencoder:
#             pairs = [[query, d.page_content] for d in docs]
#             scores = RERANKER.predict(pairs)
...
#     expanded = expand_with_neighbors(best_docs, doc_store, window=neighbor_window)
#     merged_text = "\n".join([d.page_content for d in expanded])
#     final_doc = Document(page_content=merged_text, metadata={"source_type": "pdf"})

#     # Step 5: unified LLM generation (like video)
#     ans_dict = answer_from_docs(query, [final_doc], llm, source_type="pdf")
#     snippet = ans_dict["kb"]
#     print("****PDF Snippet*****", snippet)

#     # Step 6: handle "no answer" case
#     if snippet == "The answer is not present in the pdfs.":
#         return out

#     # Step 7: save QA to memory
#     add_to_QA_index(query, snippet, "pdf", embeddings)

#     # Step 8: finalize output (no sources/timestamps needed)
#     out["answer"] = snippet
#     return out
...
# if "vs_combined" not in st.session_state:
#     st.session_state.vs_combined, st.session_state.vs_combined_store = load_index(INDEX_DIR_COMBINED, embeddings)
# else:
#     if isinstance(st.session_state.vs_combined, tuple):
#         st.session_state.vs_combined, st.session_state.vs_combined_store = st.session_state.vs_combined
#     else:
#         st.session_state.vs_combined_store = {}
